...
class HRBranch(nn.Module):
    ...
    def __init__(self, hr_channels, hr_channel):
        ...
        self.conv_enc2x = Conv2dIBNormRelu(hr_channels + 3, hr_channels, 3, stride=2, padding=1)  # 35,32
        ...
        self.conv_enc4x = Conv2dIBNormRelu(2 * hr_channels, 2 * hr_channels, 3, stride=1, padding=1)  # 64, 64
...
class MODNet_auto(nn.Module):
    ...
    def forward(self, img, inference=False):
        lr8x, [enc2x, enc4x] = self.lr_branch(img, inference)
        hr2x = self.hr_branch(img, enc2x, enc4x, lr8x, inference)
